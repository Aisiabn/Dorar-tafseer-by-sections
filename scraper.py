...
def extract_articles(html):
    ...
    for block in articles:
        ...
        # نقسّم عند كل title-1
        parts = TITLE1_RE.split(inner_html)
        # parts = [محتوى_قبل_أول_title1, نص_title1, محتوى_1, نص_title1, محتوى_2, ...]

        segments = [(main_heading, parts[0])]
        it = iter(parts[1:])
        for title_text, content in zip(it, it):
            heading = re.sub(r'<[^>]+>', '', title_text).strip()
            segments.append((heading, content))

        for seg_heading, raw in segments:
            if not seg_heading:
                continue
            clean, footnotes, fn_counter = process_chunk(raw, fn_counter)
            if clean:
                results.append({
                    "heading"  : seg_heading,
                    "text"     : clean,
